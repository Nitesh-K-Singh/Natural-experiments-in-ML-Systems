...
def run_simulations(df, policies, num_simulations):
    ...
    for i in range(num_simulations):
        temp_df = df.copy(
            deep=True
        )  # our features, theta, p0,p1 are same across simulations
        temp_df_po = draw_potential_outcomes(
            temp_df
        )  # this is our source of randomness across simulations
        temp_df_policies = apply_policies(
            temp_df_po, policies
        )  # this is same for all policies (except random)
        temp_df_outcomes = apply_realised_outcomes(
            temp_df_policies, policy_cols
        )  # randomness here comes from randomness in potential outcomes (above)
...
def get_summary(df):
    ...
    )  # can also get this via bootstrap
    summary["ci_upper"] = (
        summary["mean"] + 1.96 * summary["se"]
    )  # can also get this via bootstrap
